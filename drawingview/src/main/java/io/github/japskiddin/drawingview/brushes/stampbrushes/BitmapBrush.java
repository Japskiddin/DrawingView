package io.github.japskiddin.drawingview.brushes.stampbrushes;
...
public class BitmapBrush extends StampBrush {
  ...
  public void drawFromTo(Canvas canvas, float lastDrawnPoint[], float x1, float y1) {
    ...
    for (; i <= 1; i += stepInPercentage) {
      ...
      canvas.drawBitmap(mResizedBrush, xCenter - mHalfResizedBrushWidth,
          yCenter - mHalfResizedBrushHeight, null);
    }
  ...
  public void drawPoint(Canvas canvas, float left, float top) {
    canvas.drawBitmap(mResizedBrush, left - mHalfResizedBrushWidth, top - mHalfResizedBrushHeight,
        null);
  }
  ...
  public void setSizeInPercentage(float sizePercentage) {
    ...
    mResizedBrush = Bitmap.createBitmap(mSizeInPixel,
        (int) (mSizeInPixel / (float) mOriginalBrush.getWidth() * mOriginalBrush.getHeight()),
