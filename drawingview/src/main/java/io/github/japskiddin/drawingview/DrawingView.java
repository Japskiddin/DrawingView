...
import io.github.japskiddin.drawingview.brushes.BrushSettings;
import io.github.japskiddin.drawingview.brushes.Brushes;
...
public class DrawingView extends View {
  ...
  //This is used for undo/redo, if null this means the undo and redo are disabled
  ...
  protected void onDraw(Canvas canvas) {
    ...
    canvas.translate(getPaddingStart() + mDrawingTranslationX,
        getPaddingTop() + mDrawingTranslationY);
  ...
  public void setBackgroundImage(Bitmap bitmap) {
    ...
    if (mActionStack
        != null) //if undo and redo is enabled, remove the old actions by creating a new instance.
    {
      mActionStack = new ActionStack();
    }
    invalidate();
  ...
  public void resetZoom() {
    ...
    float targetSF =
        calcAppropriateScaleFactor(mDrawingBitmap.getWidth(), mDrawingBitmap.getHeight());
    ...
    ObjectAnimator scaleAnimator =
        ObjectAnimator.ofFloat(this, "scaleFactor", mScaleFactor, targetSF);
  ...
  public boolean undo() {
    if (mActionStack == null) {
      throw new IllegalStateException(
          "Undo functionality is disable you can enable it by calling setUndoAndRedoEnable(true)");
    }
  ...
  public boolean redo() {
    if (mActionStack == null) {
      throw new IllegalStateException(
          "Redo functionality is disable you can enable it by calling setUndoAndRedoEnable(true)");
    }
    ...
    try {
      ...
      if (size < 0 || size > 1) {
        throw new IllegalArgumentException(
            "DrawingView brush_size attribute should have a value between 0 and 1 in your xml file");
      }
      settings.setSelectedBrushSize(size);

      mBGColor = typedArray.getColor(R.styleable.DrawingView_drawing_background_color,
          -1);//default to white
    } finally {
      typedArray.recycle();
    }
  ...
  private void privateSetBGBitmap(Bitmap bitmap) {
    ...
    {
      bitmap = Utilities.resizeBitmap(bitmap, ((int) (bitmap.getWidth() * scaleFactor)),
  ...
  private float calcAppropriateScaleFactor(int bitmapWidth, int bitmapHeight) {
    ...
    if (bitmapWidth < canvasWidth && bitmapHeight < canvasHeight) {
      ...
      scaleFactor = canvasHeight
          / bitmapHeight;//let us zoom in to make the bitmap height appears equal to the view height
